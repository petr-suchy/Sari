...
int main()
{
...

    {
        ...
        Sari::Utils::Exchanger::Transaction trans1 = Sari::Utils::Exchanger::Transaction(ioContext);
...

    {
        ...
        selling = exchanger.asyncProduce(trans1, goods)
            .then([goods](int price) {
                std::cout << "Alice sold " << goods << " for " << price << "$.\n";
            }).fail([](boost::system::error_code ec) {
                if (ec == make_error_code(boost::system::errc::operation_canceled)) {
    ...
    Sari::Utils::Promise::All(ioContext, { selling })
        .then([]() {
            std::cout << "Business done!\n";
        }).fail([]() {
            std::cout << "Business failed!\n";
        });
